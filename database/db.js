...
const getAllQuizzes = (callback) => {
    const sql = `SELECT id, title, difficulty, user_id FROM quizzes`;
    db.all(sql, [], (err, rows) => {
        callback(err, rows);
    });
};
...
const updateQuestion = (id, text, optA, optB, optC, optD, answer, callback) => {
    const sql = `UPDATE questions 
                 SET question_text = ?, option_a = ?, option_b = ?, option_c = ?, option_d = ?, correct_answer = ? 
                 WHERE id = ?`;
    db.run(sql, [text, optA, optB, optC, optD, answer, id], callback);
};
